...
export function calculateCalories(weightKg, heightCm, age, gender, activityLevel = 'o_rtacha') {
  ...
  if (!multiplier) {
    console.warn(`Noma'lum activity_level: "${activityLevel}", default 1.55 ishlatildi`)
  }
  const finalMultiplier = multiplier || 1.55

  const maintenance = Math.round(bmr * finalMultiplier)
