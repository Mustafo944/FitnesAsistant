...
export default function AuthCallbackPage() {
  ...
  useEffect(() => {
    const handleCallback = async () => {
      ...
      if (code) {
        // Code borku, demak PKCE exchange qilish kerak
      ...
      // Sessionni tekshiramiz va dashboardga o'tamiz
      const { data: { session } } = await supabase.auth.getSession()
      
      if (session) {
        // Foydalanuvchi onboard bo'lganini tekshirish (ixtiyoriy, lekin yaxshi UX uchun)
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#08001a] relative overflow-hidden">
      ...
      <div className="relative z-10 text-center space-y-6">
        ...
        <div className="space-y-2">
          ...
          <p className="text-gray-500 text-sm animate-pulse font-medium">Sizning profilingiz tahlil qilinmoqda</p>
        </div>
      </div>
    </div>
  )
