    ...
    <PageWrapper className="max-w-2xl mx-auto py-6 md:py-8 px-4 pb-24">
      ...
      <Card glass className="mb-6 border-emerald-500/20">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-sm font-bold text-emerald-400">{logsLoading ? '...' : `${daysLogged}/${DAYS_NEEDED}`} kun</span>
        </div>
        <div className="h-2 bg-white/5 rounded-full overflow-hidden">
          <div 
            className="h-full bg-gradient-to-r from-emerald-500 to-green-400 rounded-full transition-all duration-500"
            style={{ width: `${logsLoading ? 0 : Math.min((daysLogged / DAYS_NEEDED) * 100, 100)}%` }}
          />
        </div>
        {!logsLoading && daysLogged < DAYS_NEEDED && (
